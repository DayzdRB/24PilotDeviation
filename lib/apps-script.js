...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyOLvPsUl1PId-0MJZl7PMAnY887bkxjTiVTQH9lJA2bbD566xPIvnVM8DsX3ualiAPWQ/exec';
...
function statusForError(code) {
  ...
  if (code === 'report_not_found' || code === 'report_not_public') return 404;
  if (code === 'response_already_submitted' || code === 'number_generation_failed' || code === 'duplicate_vote') return 409;
  if (
    code === 'missing_required_fields' ||
    code === 'invalid_ppd_number' ||
    code === 'invalid_case_id' ||
    code === 'invalid_body' ||
    code === 'invalid_json' ||
    code === 'invalid_vote' ||
    code === 'invalid_voter_hash'
  ) return 400;
  if (
    code === 'data_store_unconfigured' ||
